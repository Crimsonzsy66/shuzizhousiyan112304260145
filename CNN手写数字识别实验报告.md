...
模型包含以下关键组件：
- **卷积层**：2层卷积神经网络，提取图像特征
- **BatchNorm**：加速收敛，防止过拟合
- **MaxPool**：降维，减少参数
- **Dropout**：防止过拟合
- **全连接层**：128个神经元，输出10个类别
...
```
project/
├── app_v2.py              # Flask Web应用入口（v2.0新版）
├── app_hf.py              # Gradio版本（用于HuggingFace部署）
├── templates/
│   └── index_new.html     # 前端页面（新版深蓝主题）
...
项目可在本地运行，访问地址：
- **新版系统（推荐）**：`http://127.0.0.1:5001/`
- **旧版系统**：`http://127.0.0.1:5000/`

### 2.5 提交信息

| 提交项 | 内容 |
|--------|------|
| GitHub 仓库地址 | https://github.com/Crimsonzsy66/shuzizhousiyan112304260145 |
| 在线访问链接（本地） | http://127.0.0.1:5001/ |
...
- [x] Web应用代码（app_v2.py + templates/index_new.html）
- [x] 在线访问链接
- [x] 页面截图与预测结果截图
